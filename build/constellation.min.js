...
DirectGraphView.prototype.la=function(){var a,b=this.result.ba(),c=this.result.aa(),d=this.source.R(),e=this.source.oa();for(a=0;a<e.length;a++)index=c.indexOf(e[a].id),0<=index&&(edge=this.result.A(e[a].id),edge.data=e[a].data,c.splice(index,1),e.splice(a,1),a--);for(a=0;a<d.length;a++)index=b.indexOf(d[a].id),0<=index&&(node=this.result.k(d[a].id),node.data=d[a].data,b.splice(index,1),d.splice(a,1),a--);for(a=0;a<c.length;a++)this.result.U(c[a]);for(a=0;a<b.length;a++)this.result.removeNode(b[a]);
...
Layout.prototype.M=function(){for(var a=this.constellation.R(),b=0;b<a.length;b++){var c=a[b];if(c.x==s||c.y==s)c.x=10*Math.floor((100*Math.random()-50)/10),c.y=10*Math.floor((100*Math.random()-50)/10)}jQuery(this).trigger("change")};Layout.prototype.viewChanged=Layout.prototype.M;StaticLayout=function(a){0>=arguments.length||(Layout.call(this,a),this.p=s)};window.StaticLayout=StaticLayout;StaticLayout.prototype=new Layout;
...
{context:this},function(a){a.data.context.Pa(a)})};StaticLayout.prototype.setConstellation=StaticLayout.prototype.j;StaticLayout.prototype.M=function(){for(var a=this.constellation.R(),b=0;b<a.length;b++){var c=a[b];if(c.x==s||c.y==s)c.x=isNaN(c.data.x)?(Math.random()-0.5)*this.constellation.Q:Number(c.data.x),c.y=isNaN(c.data.y)?(Math.random()-0.5)*this.constellation.N:Number(c.data.y)}jQuery(this).trigger("change")};StaticLayout.prototype.viewChanged=StaticLayout.prototype.M;
...
StaticLayout.prototype.step=function(){for(var a in this.constellation.h){var b=this.constellation.h[a];if(b.ga){var c=b.i,d=b.ga,e=this.constellation.F(c.pageX)-b.Ma,b=this.constellation.G(0,c.pageY)-b.Na;d.x=this.constellation.O(e,b);d.y=this.constellation.P(e,b)}}jQuery(this).trigger("change");this.p&&clearTimeout(this.p);this.p=setTimeout(function(a){return function(){a.step()}}(this),40)};
...
o=-b*(j.x+j.width/2),b=-b*(j.y+j.height/2),j=this.constellation.R(),k=0;k<j.length;k++)for(var m=j[k],l=k+1;l<j.length;l++){var n=j[l],p=n.x-m.x,r=n.y-m.y;0==p&&0==r&&(p=0.5*Math.random(),r=0.5*Math.random());for(var v=Math.sqrt(p*p+r*r),x=c,D=p-p/v*x,E=r-r/v*x,p=r=0,F=t,B=0;B<m.edges.length;B++){var G=m.edges[B];if(G.tailNode.id==n.id||G.headNode.id==n.id)F=q}F&&v>x?(r=D*d*a,p=E*d*a):v<x&&(r=D*e*a,p=E*e*a);v=1;x=m.edges.length;0<x&&(v=1/Math.pow(x,1/3));m.ax+=r*v;m.ay+=p*v;v=1;x=n.edges.length;0<
x&&(v=1/Math.pow(x,1/3));n.ax-=r*v;n.ay-=p*v}for(k=0;k<j.length;k++)c=j[k],d=Math.sqrt(c.ax*c.ax+c.ay*c.ay),d>i&&(c.ax=c.ax/d*i,c.ay=c.ay/d*i),c.ax-=c.vx*g,c.ay-=c.vy*g,c.vx+=c.ax,c.vy+=c.ay,c.x+=c.vx+o,c.y+=c.vy+b,h&&(d=c.S().getBBox(),c.x-d.width/2<this.viewport.x&&(c.x=this.viewport.x+d.width/2,c.vx*=-1),c.y-d.height/2<this.viewport.y&&(c.y=this.viewport.y+d.height/2,c.vy*=-1),c.x+d.width/2>this.viewport.x+this.viewport.width&&(c.x=this.viewport.x-d.width/2+this.viewport.width,c.vx*=-1),c.y+d.height/
2>this.viewport.y+this.viewport.height&&(c.y=this.viewport.y-d.height/2+this.viewport.height,c.vy*=-1)),c.ax=0,c.ay=0;for(var I in this.constellation.h)h=this.constellation.h[I],h.ga&&(o=h.i,i=h.ga,g=this.constellation.F(o.pageX)-h.Ma,h=this.constellation.G(0,o.pageY)-h.Na,i.x=this.constellation.O(g,h),i.y=this.constellation.P(g,h));jQuery(this).trigger("change");0==a?this.stop():this.sa&&(this.p&&clearTimeout(this.p),this.p=setTimeout(function(a){return function(){a.step()}}(this),40));this.ka++};
RoamerLayout.prototype.Qb=function(a){for(var b=0;b<a.length;b++)this.setNodeInitialPosition(a[b])};RoamerLayout.prototype.setNodeInitialPositions=RoamerLayout.prototype.Qb;RoamerLayout.prototype.Pb=function(a){var b=0,c=0,d=a.Ba().filter(this.Za),e=y(d);1<d.length?(b=e.x,c=e.y):0<d.length&&(c=d[0],b=c.Ba().filter(this.Za),0<b.length?(b=y(b),d={x:c.x-b.x,y:c.y-b.y},e=Math.sqrt(d.x*d.x+d.y*d.y),b=c.x+50*d.x/e,c=c.y+50*d.y/e):(b=c.x,c=c.y));a.x=b+Math.random()-0.5;a.y=c+Math.random()-0.5};
RoamerLayout.prototype.setNodeInitialPosition=RoamerLayout.prototype.Pb;RoamerLayout.prototype.Za=function(a){return a.x!=s||a.y!=s};function y(a){if(0>=a.length)return s;for(var b=0,c=0,d=0;d<a.length;d++)var e=a[d],b=b+e.x,c=c+e.y;return{x:b/a.length,y:c/a.length}}NodeRenderer=function(a,b,c){this.constellation=a;this.classes=[];Node.call(this,b,c);this.t();this.y=this.x=s;this.ay=this.ax=this.vy=this.vx=0};window.NodeRenderer=NodeRenderer;NodeRenderer.prototype=new Node;
...
NodeRenderer.prototype.removeClass=function(a){for(var b;0<=(b=jQuery.inArray(a,this.classes));)this.classes.splice(b,1)};NodeRenderer.prototype.removeClass=NodeRenderer.prototype.removeClass;NodeRenderer.prototype.t=function(){Node.prototype.t.call(this);if(this.data["class"]){this.classes=[];for(var a=this.data["class"].split(/\s/),b=0;b<a.length;b++)this.addClass(a[b])}};NodeRenderer.prototype.dataChanged=NodeRenderer.prototype.t;NodeRenderer.prototype.create=u();
NodeRenderer.prototype.create=NodeRenderer.prototype.create;NodeRenderer.prototype.q=u();NodeRenderer.prototype.draw=NodeRenderer.prototype.q;NodeRenderer.prototype.position=u();NodeRenderer.prototype.position=NodeRenderer.prototype.position;NodeRenderer.prototype.J=u();NodeRenderer.prototype.destroy=NodeRenderer.prototype.J;NodeRenderer.prototype.S=u();NodeRenderer.prototype.getSvg=NodeRenderer.prototype.S;
DefaultNodeRenderer=function(a,b,c){NodeRenderer.call(this,a,b,c);this.Oa=this.Ka=this.cb=this.bb=this.L=this.label=s};window.DefaultNodeRenderer=DefaultNodeRenderer;DefaultNodeRenderer.prototype=new NodeRenderer;
...
DefaultEdgeRenderer.prototype.create=function(){var a=this.constellation.svg,b=a.group(this.constellation.$a());this.a={group:b,line:a.line(b,0,0,10,0,{stroke:this.c("edgeLineColor"),strokeWidth:this.c("edgeLineThickness")}),l:s,m:s,L:a.title(b,"")};jQuery(this.a.group).bind("mouseover",{context:this},function(a){a.data.context.constellation.edgemouseoverHandler(a,a.data.context)}).bind("mouseout",{context:this},function(a){a.data.context.constellation.edgemouseoutHandler(a,a.data.context)}).bind("mousedown",
...
DefaultEdgeRenderer.prototype.position=DefaultEdgeRenderer.prototype.position;DefaultEdgeRenderer.prototype.J=function(){jQuery(this.a.line).remove();this.a.l&&jQuery(this.a.l).remove();this.a.m&&jQuery(this.a.m).remove()};DefaultEdgeRenderer.prototype.destroy=DefaultEdgeRenderer.prototype.J;DefaultEdgeRenderer.prototype.S=function(){return this.a.group};
DefaultEdgeRenderer.prototype.Ea=function(){for(var a="";0<arguments.length;)var b=Array.prototype.shift.call(arguments),a=a+(" "+b[0]+","+b[1]);return a};DefaultEdgeRenderer.prototype.getPointsString=DefaultEdgeRenderer.prototype.Ea;Constellation=function(a,b){this.config=jQuery.extend(q,this.ob,a);this.ua=b?this.Ya.concat(b):this.Ya;this.f=[];this.e=[];this.xa=[];this.wa=[];this.svg=s;this.z=this.w=this.N=this.Q=0;this.n=1;this.rotation=0;this.h={};this.V=this.u=this.s=this.d=s};
...
Constellation.prototype.k=function(a){for(var b=0;b<this.f.length;b++)if(this.f[b].id==a)return this.f[b];return s};Constellation.prototype.getNode=Constellation.prototype.k;Constellation.prototype.R=function(){return this.f.concat()};Constellation.prototype.getNodes=Constellation.prototype.R;Constellation.prototype.ba=function(){return this.f.map(function(a){return a.id})};Constellation.prototype.getNodeIds=Constellation.prototype.ba;Constellation.prototype.Da=function(){return this.f.length};
...
Constellation.prototype.La=function(){this.debug("Model changed");this.o&&this.o.mb();jQuery(this).trigger("modelchanged")};Constellation.prototype.modelChanged=Constellation.prototype.La;Constellation.prototype.M=function(){this.debug("View changed");this.K&&this.K.viewChanged();jQuery(this).trigger("viewchanged")};Constellation.prototype.viewChanged=Constellation.prototype.M;Constellation.prototype.fb=function(){this.q()};Constellation.prototype.layoutChanged=Constellation.prototype.fb;
Constellation.prototype.q=function(){var a,b;for(z(this);0<this.xa.length;)b=this.xa.pop(),b.draw();for(;0<this.wa.length;)b=this.wa.pop(),b.draw();var c=this.f;for(a=0;a<c.length;a++)b=c[a],b.position();c=this.e;for(a=0;a<c.length;a++)b=c[a],b.position()};function z(a){a.Q=a.v.width();a.N=a.v.height();A(a);jQuery(a).trigger("viewportchange")}
function A(a){a.v.button&&a.v.slider&&jQuery(a.config.selector+" .zoomSlider").slider("option","value",a.n);jQuery(a.Va).attr("transform","translate("+(a.Q/2-a.w)+","+(a.N/2-a.z)+")scale("+a.n+","+a.n+")rotate("+180*a.rotation/Math.PI+")")}Constellation.prototype.pa=w("n");Constellation.prototype.getZoomScale=Constellation.prototype.pa;Constellation.prototype.W=function(a){this.n=a;A(this);jQuery(this).trigger("viewportchange")};Constellation.prototype.setZoomScale=Constellation.prototype.W;
Constellation.prototype.Fa=w("rotation");Constellation.prototype.getRotation=Constellation.prototype.Fa;Constellation.prototype.Tb=function(a){this.rotation=a;A(this);jQuery(this).trigger("viewportchange")};Constellation.prototype.setRotation=Constellation.prototype.Tb;Constellation.prototype.O=function(a,b){return(a-this.Q/2+this.w)/this.n*Math.cos(-this.rotation)-(b-this.N/2+this.z)/this.n*Math.sin(-this.rotation)};Constellation.prototype.viewportToWorldX=Constellation.prototype.O;
Constellation.prototype.P=function(a,b){return(a-this.Q/2+this.w)/this.n*Math.sin(-this.rotation)+(b-this.N/2+this.z)/this.n*Math.cos(-this.rotation)};Constellation.prototype.viewportToWorldY=Constellation.prototype.P;Constellation.prototype.Y=function(a,b){return(a*Math.cos(this.rotation)-b*Math.sin(this.rotation))*this.n+this.Q/2-this.w};Constellation.prototype.worldToViewportX=Constellation.prototype.Y;
Constellation.prototype.Z=function(a,b){return(a*Math.sin(this.rotation)+b*Math.cos(this.rotation))*this.n+this.N/2-this.z};Constellation.prototype.worldToViewportY=Constellation.prototype.Z;Constellation.prototype.F=function(a){return a-this.v.offset().left};Constellation.prototype.pageToViewportX=Constellation.prototype.F;Constellation.prototype.G=function(a,b){return b-this.v.offset().top};Constellation.prototype.pageToViewportY=Constellation.prototype.G;
Constellation.prototype.Ha=function(){var a={x:this.O(0,0),y:this.P(0,0)};a.width=this.O(this.Q,this.N)-a.x;a.height=this.P(this.Q,this.N)-a.y;return a};Constellation.prototype.getViewportBounds=Constellation.prototype.Ha;Constellation.prototype.Lb=function(a,b){a.stopPropagation();a.preventDefault();this.ma(b);jQuery(this).trigger("nodemouseover",b.id)};Constellation.prototype.nodemouseoverHandler=Constellation.prototype.Lb;
Constellation.prototype.Kb=function(a,b){a.stopPropagation();a.preventDefault();jQuery(this).trigger("nodemouseout",b.id)};Constellation.prototype.nodemouseoutHandler=Constellation.prototype.Kb;Constellation.prototype.ha=function(a,b){a.stopPropagation();a.preventDefault();this.ma(b);var c=this.h._mouse={ca:t,ga:b,i:a,pageX:a.pageX,pageY:a.pageY,timestamp:(new Date).getTime()},d=this.F(a.pageX),e=this.G(0,a.pageY);c.Ma=this.O(d,e)-b.x;c.Na=this.P(d,e)-b.y;jQuery(this).trigger("nodemousedown",b.id)};
